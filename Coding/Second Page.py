...
for meter in df["Meter"].unique():
    ...
    if not shop_info:
        print(f"Warning: no tariff info for {shop_no}, skipping {meter}")
        continue
    ...
    # ================== Daily aggregation with fixed date range ==================
    ...
    if pd.notna(anytime_rate):
        # Anytime / tiered tariff (threshold: 1650 kWh per day)
        ...
        # Time-of-use tariff: classify each interval as peak or off-peak
    ...
    print(f"Done: {output_file} (Tariff = {tariff_type})")
