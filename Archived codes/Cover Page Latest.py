...
def calculate_usage(df, meter, shop_info):
    ...
    # 🔹 强制写死 31 天
    days = 31
    start_time = pd.Timestamp("2025-07-17")
    end_time = pd.Timestamp("2025-08-17")
...
def generate_cover_page(pdf_path, usage_data, shop_info, meter, statement_date="2025/07/22"):
    ...
    c.drawString(40, height-190, "Prepared by Element47 Pty Ltd on behalf of:")
    c.setFont("Helvetica", 9)
    c.drawString(40, height-205, "The Plaza Centre, 38 Mandurah Terrace, Mandurah 6210")
    ...
    if usage_data.get("peak_rate") is not None:
        ...
        c.drawString(60, y-60, "Billing Days: 31 days")
    else:
...
def generate_all_covers(results_file, mapping_file, output_folder):
    ...
    for meter in df["Meter"].unique():
        ...
        if shop_no in ["Common Service", "MDB"]:
            shop_info = mapping.get("11", {}).copy()  # 假设用 Shop2 的 tariff
        else:
